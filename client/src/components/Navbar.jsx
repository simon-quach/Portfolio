...
const Navbar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <motion.div
      className={`transition-all duration-500 sticky ${
        scrollDirection === "down" ? "-top-72" : "top-0"
      } h-[72px] w-full px-[20px] flex justify-center items-center bg-white shadow-sm z-10`}
      initial={{ opacity: 0 }}
      whileInView={{
        opacity: 1,
        transition: {
    ...
    >
      <div className="w-full h-full flex justify-between items-center text-center">
        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          ...
          >
            <img
              className="w-[56px] cursor-pointer ml-[-4px]"
